...
class HookDocsGenerator
{
     ...
     * Source path.
     */
    protected const SOURCE_PATH = '';

    /**
     * Hooks template path.
     */
    protected const HOOKS_TEMPLATE_PATH = 'docs/api/hooks/hooks.html';

    /**
     * Search index path.
     */
    protected const SEARCH_INDEX_PATH = 'docs/api/js/searchIndex.js';
    ...
    protected static function getFilesToScan(): array
    {
        ...
        $files['Template Functions'] = array( self::SOURCE_PATH . 'includes/functions.php', self::SOURCE_PATH . 'includes/hooks.php' );
        $files['Core Classes']         = self::getFiles('*.php', GLOB_MARK, self::SOURCE_PATH . 'includes/');
        $files['Email classes']         = self::getFiles('*.php', GLOB_MARK, self::SOURCE_PATH . 'includes/emails/');
        $files['Product Classes']         = self::getFiles('*.php', GLOB_MARK, self::SOURCE_PATH . 'includes/products/');
    ...
    protected static function getJSOutput(array $hook_list): string
    {
        ...
        foreach ($hook_list as $heading => $hooks) {
            foreach ($hooks as $hook => $details) {
                ...
                foreach ($details['files'] as $file) {
                    $summary .= ' located in ' . str_replace('eeco-gift-cards/', '', $file['path']) . ': ' . $file['line'];
